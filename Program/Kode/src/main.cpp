...
void runMachineLearningInference() {
    ...
    // Array Fitur: [Raw Sensor, Suhu, Kelembapan]
    float input_pm[3] = { g_data.raw_pm_adc, g_data.suhu, g_data.kelembapan };
...
void sendSerialTelemetry() {
    // Format Serial CSV Logger:
    // Suhu,Kelembapan,PM25_Calib,CO_Calib,ISPU_Final,Kategori,ParamKritis,Kipas%
    Serial.printf("[TELEMETRI] T:%.1f C | RH:%.1f %% | PM2.5:%.1f ug/m3 | CO:%.2f ppm | ISPU:%d (%s) | Dominan:%s | Kipas:%d %%\r\n",
                  g_data.suhu,
                  g_data.kelembapan,
